...
def generate_3d_directions(num_thetas=16):
    indices = torch.arange(0, num_thetas, dtype=torch.float32) + 0.5
    phi = torch.acos(1 - 2 * indices / num_thetas)
...
def get_part_groups():
    return [
        [0, 1, 20, 2, 3],       # Torso
        [23, 24, 11, 10, 9, 8], # Left Arm
        [19, 18, 17, 16, 7, 6], # Right Arm
        [15, 14, 13, 12],       # Left Leg
        [21, 22, 5, 4]          # Right Leg
    ]

class Local_ECT_Gconv(nn.Module):
    def __init__(self, in_channels, out_channels, n_directions=32, resolution=32):
        super(Local_ECT_Gconv, self).__init__()
        self.groups = get_part_groups()
        
        init_directions = generate_3d_directions(n_directions)
        self.raw_directions = nn.Parameter(init_directions)
        
        thresholds = torch.linspace(-2.0, 2.0, resolution).view(1, 1, 1, resolution)
        self.register_buffer('thresholds', thresholds)
        self.resolution = resolution
        
        ect_dim = n_directions * resolution
        self.project = nn.Sequential(
            nn.Linear(ect_dim, out_channels),
    ...
    def forward(self, x):
        N, C, T, V = x.size()
        directions = F.normalize(self.raw_directions, p=2, dim=0)
        out = self.down(x)
        ...
        for group_idx in self.groups:
            ...
            points = part_x.permute(0, 2, 3, 1)
            points = points - points.mean(dim=2, keepdim=True)
            
          
            heights = torch.matmul(points, directions)
            h = heights.unsqueeze(-1)
            t = self.thresholds.view(1, 1, 1, 1, self.resolution)
            curve = torch.sigmoid(10.0 * (t - h)).sum(dim=2)
            
            feat = self.project(curve.view(N * T, -1)).view(N, -1, T, 1)
            part_feats.append(feat)
            
       
        ect_out = torch.zeros_like(out)
        for i, group_idx in enumerate(self.groups):
            ect_out[:, :, :, group_idx] += part_feats[i].expand(-1, -1, -1, len(group_idx))
            
        return self.relu(self.bn(out + ect_out))


class Global_ECT_Stream(nn.Module):
    def __init__(self, out_channels, n_directions=16, resolution=24):
        super(Global_ECT_Stream, self).__init__()
        self.resolution = resolution
        init_directions = generate_3d_directions(n_directions)
        self.raw_directions = nn.Parameter(init_directions) 
        thresholds = torch.linspace(-2.0, 2.0, resolution).view(1, 1, 1, resolution)
        ...
        ect_input_dim = n_directions * resolution
        
        self.mlp = nn.Sequential(
            nn.Linear(ect_input_dim, out_channels // 4),
    ...
    def forward(self, x):
        ...
        feat = self.mlp(curve.view(curve.size(0)*curve.size(1), -1))
        return feat.view(curve.size(0), curve.size(1), -1).mean(dim=1)
...
class TemporalConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1):
        ...
        self.conv = nn.Conv2d(
            ...
            padding=(pad, 0), stride=(stride, 1), dilation=(dilation, 1), bias=False)
...
class residual_conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0), stride=(stride, 1), bias=False)
        self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        conv_init(self.conv)
        bn_init(self.bn, 1)
...
class EdgeConv(nn.Module):
    ...
    def get_graph_feature(self, x, k, idx=None):
        N, C, V = x.size()
        if idx is None: idx = self.knn(x, k=k)
        device = x.get_device()
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, kernel_size=5, dilations=[1, 2], att=True, CoM=21):
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, ect_type='fusion'):
        ...
        print(">>> [Architecture] Replacing Layer 1 with Local_ECT_Gconv (Part-based, Learnable)...")
        self.l1.gcn1 = Local_ECT_Gconv(in_channels=3, out_channels=base_channels, n_directions=32, resolution=32)
        ...
        final_dim = base_channels * 4
        self.global_ect = Global_ECT_Stream(out_channels=final_dim, n_directions=16, resolution=24)

        self.fc = nn.Linear(final_dim, num_class)
    ...
    def forward(self, x):
        ...
        x_raw = x.mean(dim=4) # (N, 3, T, V)
        global_ect_feat = self.global_ect(x_raw) # (N, 256)
