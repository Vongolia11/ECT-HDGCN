...
def generate_3d_directions(num_thetas=16):
    ...
    directions = torch.stack([x, y, z], dim=0) # [3, N]
    return directions

class ECT_Stream(nn.Module):
    def __init__(self, out_channels, n_directions=16, resolution=24):
        super(ECT_Stream, self).__init__()
        
        self.out_channels = out_channels
        self.n_directions = n_directions
        self.resolution = resolution
        
        directions = self._generate_3d_directions(n_directions)
        ...
        self.mlp = nn.Sequential(
            nn.Linear(ect_input_dim, out_channels),
    ...
    def _generate_3d_directions(self, n):
    ...
    def forward(self, x):
        ...
        center = points.mean(dim=2, keepdim=True) # (N, T, 1, 3)
        points = points - center
        
        # points: (N, T, V, 3)
        # directions: (3, D)
        # heights = points @ directions -> (N, T, V, D)
        heights = torch.matmul(points, self.directions)
        
        
        h = heights.unsqueeze(-1)
        t = self.thresholds.view(1, 1, 1, 1, self.resolution)
        
        # height <= threshold  <==>  threshold - height >= 0
        scale = 10.0
        diff = t - h
        curve_val = torch.sigmoid(scale * diff)
        
        ect = curve_val.sum(dim=2)
        
        ect_flat = ect.view(N * T, -1)
        
        feat = self.mlp(ect_flat)
        
        feat = feat.view(N, T, -1)
        out = feat.mean(dim=1)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.ect_stream = ECT_Stream(out_channels=base_channels*4, n_directions=16, resolution=24)
    ...
    def forward(self, x):
        ...
        ect_feature = self.ect_stream(x_raw) # (N, 256)
        ...
        x = x.mean(3).mean(1) # Global Pooling -> (N, 256)
